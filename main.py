...
# Python, void functions are not exactly the same as functions in C/C++
...
# NO need to specify the data types of variables as the interpreter itself predicts the variable data type based on the value assigned to it
# Yet still can convert signed integer to unsigned integer with the following example
signed_integer = -100
...
opening() # Python is known as Escape Characters(Escape sequences in C++)
text1 = "Hello"
text2 = "There"
...
opening()
# Numeric function
# More often used, standard operators such as +, -, *, /. pow for getting power value
